...
object TestLexer {
  ...
  def main(args: Array[String]): Unit = {
    ...
    while (true) {
      ...
      println("simplified: " +
        node.accept(new ConstantSimplifyVisitor).asInstanceOf[Node]
          .accept(new StringerVisitor))
      println("differentiated: " +
        node.accept(new DifferentiateVisitor).asInstanceOf[Node]
          .accept(new StringerVisitor))
      println("differentiate simplified: " +
        node.accept(new DifferentiateVisitor).asInstanceOf[Node]
          .accept(new ConstantSimplifyVisitor).asInstanceOf[Node]
          .accept(new StringerVisitor))
    }
  }
}
